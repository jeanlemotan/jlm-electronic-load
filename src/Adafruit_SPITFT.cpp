...
void Adafruit_SPITFT::writeVPattern(uint8_t pattern, uint8_t length, int16_t x, int16_t y, uint16_t color1, uint16_t color2, uint8_t size)
{
    if (color1 == color2) {
        Adafruit_GFX::writeVPattern(pattern, length, x, y, color1, color2, size);
        return;
    }

    int16_t w = size, h = length * size;
    ...
    uint16_t c[2] = { color2, color1 };
    uint8_t sq = w*size;
    for (uint8_t i=0; i<length; i++, pattern >>= 1) {
        writeColor(c[pattern & 1], sq);
    }
}

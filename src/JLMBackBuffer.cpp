...
void JLMBackBuffer::writePixel(int16_t x, int16_t y, uint16_t color, uint8_t alpha)
{
...
void JLMBackBuffer::writePixel(int16_t x, int16_t y, uint16_t color)
{
  if (_opacity <= 223)
  {
    return writePixel(x, y, color, 0xFF);
  }
...
void JLMBackBuffer::drawAALine(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
   ...
   if (dy > dx) 
   {
      ...
      while (--dy) 
      {
         ...
         y1++;
         writePixel(x1, y1, color, 0xFF - (errorAcc >> 8));
         writePixel(x1 + xdir, y1, color, (errorAcc >> 8));
      }
   ...
   while (--dx) 
   {
      ...
      x1 += xdir;
      writePixel(x1, y1, color, 0xFF - (errorAcc >> 8));
      writePixel(x1, y1 + 1, color, (errorAcc >> 8));
   }
   writePixel(x2, y2, color);
}
...
{                    
   ...
   do 
   {
      ...
      writePixel(x0-x, y0+y, color, i);                             //   I. Quadrant
      writePixel(x0-y, y0-x, color, i);                             //  II. Quadrant
      writePixel(x0+x, y0-y, color, i);                             // III. Quadrant
      writePixel(x0+y, y0+x, color, i);                             //  IV. Quadrant
      int16_t e2 = err; 
      int16_t x2 = x;
      if (err+y > 0) 
      {                                                             // x step
         ...
         if (i > 0) 
         {
            writePixel(x0-x, y0+y+1, color, i);
            writePixel(x0-y-1, y0-x, color, i);
            writePixel(x0+x, y0-y-1, color, i);
            writePixel(x0+y+1, y0+x, color, i);
         }
         err += ++x*2+1;
      ...
      {                                                             // y step
         ...
         if (i > 0) 
         {
            writePixel(x0-x2-1, y0+y, color, i);
            writePixel(x0-y, y0-x2-1, color, i);
            writePixel(x0+x2+1, y0-y, color, i);
            writePixel(x0+y, y0+x2+1, color, i);
         }
         err += ++y*2+1;
